...
数据到位后只需训练模型头、校准阈值和评估，不再重构基础工程。

> 状态：无数据阶段完成（P0–P8），141 个自动测试通过。
> - 不知道文件作用 → [`PROJECT_MAP.md`](PROJECT_MAP.md)
> - 不知道做到哪了 → [`CURRENT_STATE.md`](CURRENT_STATE.md)
> - Agent 工作规范 → [`AGENTS.md`](AGENTS.md)
> - 执行方案 → `TODO.md`；验收报告 → [`reports/no_data_stage_report.md`](reports/no_data_stage_report.md)

## 资产

- `chinese-wwm-roberta.ckpt`：微调 checkpoint（199 个 `bert.*` backbone 键 + `fc.weight [2,768]` + `fc.bias [2]`）。
- `chinese-roberta-wwm-ext/`：本地预训练底座（含 `model.safetensors` 安全版权重）。
- 全部哈希/大小/结构清单：`metadata/model_manifest.json`（`scripts/verify_assets.py` 生成）。

## 工程结构

```text
docs/          WORKFLOW.md / MODEL.md / EXPERIMENTS.md / PLAN.md
configs/       model / heads / early_exit / benchmark / factor 配置
metadata/      model_manifest.json / data_schema.json / factor_schema.json
src/
  models/      checkpoint, modeling, pooling, early_exit
  probes/      heads, layer_outputs, training
  data/        dataset.py + fetch/（外部数据源取数脚本）
  factors/     factor
  evaluation/  analysis, calibration
  config.py    共享 YAML+环境变量配置加载
scripts/       verify_assets / smoke_inference / report_heads / export_layer_outputs /
               demo_early_exit / benchmark_fixed_exit / compare_layers /
               run_synthetic_pipeline / demo_factor / convert_to_safetensors
notebooks/     check.ipynb（探索记录）等
tests/         141 个 pytest 用例（CPU；CUDA 有 marker 待补）
...
```bash
conda activate nlp_fjq          # torch 2.1.2 / transformers 4.51.3 / CUDA A100
...
见 `CURRENT_STATE.md` 与 `reports/no_data_stage_report.md` §14.6：
